...
<table class="shop_table woocommerce-checkout-review-order-table">
    ...
    <tfoot>
        ...
        <?php /** * Выбранный кастомный способ доставки */
        $selected_delivery_method = '';
        if (!empty($_POST['custom_delivery_method'])) {
            $selected_method_id = sanitize_text_field(wp_unslash($_POST['custom_delivery_method']));
                ...
                foreach ($custom_delivery_regions as $region_index => $region) {
                    ...
                    foreach ($methods as $method_index => $method) {
                        ...
                        if ($selected_method_id === $method_id) {
                            $selected_delivery_method = $method['custom_delivery_method_name'] ?? '';
                            break 2;
                        }
                    }
                }
            }
        } ?> <?php if ($selected_delivery_method) : ?> <tr class="custom-delivery-method">
                <th>Способ доставки</th>
                <td> <?php echo esc_html($selected_delivery_method); ?> </td>
            </tr> <?php endif; ?>
